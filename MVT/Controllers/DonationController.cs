...
namespace MVT.Controllers
{
    ...
    public class DonationController : ApiController
    {
        ...
        [Route("api/MonthlyDonation/{id}")]
        [HttpGet]
        public List<int> MonthlyDonation(long id)
        {
            List<int> lst = db.Donations.Where(f => f.IsActive == true && f.ProjectId == id).GroupBy(f => f.Date.Month).Select(f => f.Sum(i => i.Ammount)).ToList<int>();
            return lst;
        }
